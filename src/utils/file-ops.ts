 ...
 */
export async function writeFile(filepath: string, content: string, force: boolean = false, backup: boolean = false): Promise<boolean | string> {
 ...
 */
export async function deleteFile(filepath: string, force: boolean = false): Promise<boolean> {
  const filePath = path.resolve(filepath);
